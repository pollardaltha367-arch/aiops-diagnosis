...
from diagnosis_engine import UNKNOWN_SEVERE, diagnose
...
def run(dataset_path: Path) -> dict:
    ...
    redaction_expected = redaction_found = open_set_hits = 0
    ...
    for case in cases:
        ...
        all_predicted = set(report["category_counts"])
        predicted = {category for category in all_predicted if category != UNKNOWN_SEVERE}
        ...
        if expected:
            open_set_hits += int(bool(all_predicted))
    ...
    return {
        ...
        "open_set_fault_recall": safe_div(open_set_hits, fault_cases),

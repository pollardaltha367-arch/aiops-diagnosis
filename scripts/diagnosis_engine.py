...
def _event_from_parts(line: int, end_line: int, message: str, level: str = "", time: str = "", host: str = "", service: str = "", stacktrace: list[str] | None = None) -> Event | None:
    ...
    is_recovered = any(pattern in message.casefold() for pattern in RECOVERY_PATTERNS)
    if not level_match and not categories and not is_recovered:
        return None
    ...
    return Event(
        ...
        state="recovered" if is_recovered else ("active" if categories else "uncertain"),
...
def to_markdown(report: dict) -> str:
    ...
    lines += ["", "## 6. 数据脱敏", ""] + [f"- {name}：{count} 处" for name, count in report["redaction"].items()]
    return "\n".join(lines) + "\n"
